...
exports.crearHoras = async (req, res) => {
  try {
    const horas = new Horas(req.body);
    await horas.save();
    res.status(201).json({ msg: '✅ Horas creadas correctamente' });
  } catch (err) {
    res.status(500).json({ error: '❌ Error al crear las horas', detalle: err.message });
  }
};
...
exports.obtenerHoras = async (req, res) => {
  try {
    ...
    res.status(500).json({ error: '❌ Error al obtener las horas', detalle: err.message });
  }
};
